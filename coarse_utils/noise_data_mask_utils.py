...
def random_choose_in_mask(ann, rand_type, **rand_kwargs):
    x1, y1, w, h = ann['bbox']
...
def add_rand_points_in_mask(jd, rand_type='uniform', rand_kwargs={}):
    anns = jd['annotations']
    offset = np.random.uniform(-0.5, 0.5, size=(len(anns), 2))
    for i, ann in tqdm(enumerate(anns)):
        x, y = random_choose_in_mask(ann, rand_type, **rand_kwargs)
        x1, y1, w, h = ann['bbox']
        assert x1 - 1 < x < x1 + w + 1 and y1 - 1 < y < y1 + h + 1, f"{[x1, y1, x1 + w, y1 + h]} {x, y}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('task', help='task, can be "generate_noisept_dataset"')
